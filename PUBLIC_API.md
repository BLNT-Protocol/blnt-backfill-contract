...
| `swap_blnd_for_blnt(from: Address, to: Address, amount: i128) -> i128` | `from` | Cumulative successful BLND-to-BLNT swap total | Before the immutable 270-day deadline, transfers and burns exactly `amount` legacy BLND, then transfers exactly the same raw amount of pre-funded BLNT to `to`. The aggregate conversion cap is 120 million BLNT. |
| `burn_expired() -> i128` | None | Expired BLNT reserve burned by this call | At or after the conversion deadline, burns the unused conversion reserve while preserving all outstanding backfill and grant claims. Returns zero after the reserve was fully converted or burned. |
...
| `get_remaining_swap_capacity() -> i128` | Remaining amount under the 120-million-BLNT aggregate conversion cap before expiry; zero at or after expiry. |
| `get_swap_deadline() -> u64` | Immutable conversion deadline, exactly 270 days after construction. |
| `get_expired_swap_burned() -> i128` | BLNT destroyed from the expired conversion reserve. |
...
| `burn_expired` | None | `amount: i128`, `total_swapped: i128` | `burn_expired` |
...
| 1208 | `SwapCapExceeded` | The swap would exceed the 120-million-BLNT aggregate cap. |
